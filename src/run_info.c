...
int RAI_ShallowCloneDagOp(RAI_DagOp *input, RAI_DagOp **output) {
  ...
  dagOp->commandType = input->commandType;
  if (input->runkey) {
    RedisModule_RetainString(NULL, input->runkey);
  }
  dagOp->runkey = input->runkey;

  dagOp->inkeys = (RedisModuleString **)array_new(RedisModuleString *, array_len(input->inkeys));
  if (!(dagOp->inkeys)) {
    return REDISMODULE_ERR;
  }
  for (int i=0; i<array_len(input->inkeys); i++) {
    dagOp->inkeys = array_append(dagOp->inkeys, RedisModule_CreateStringFromString(NULL, input->inkeys[i]));
    // RedisModule_RetainString(NULL, input->inkeys[i]);
    // dagOp->inkeys = array_append(dagOp->inkeys, input->inkeys[i]);
  }

  dagOp->outkeys = (RedisModuleString **)array_new(RedisModuleString *, array_len(input->outkeys));
  if (!(dagOp->outkeys)) {
    return REDISMODULE_ERR;
  }
  for (int i=0; i<array_len(input->outkeys); i++) {
    dagOp->outkeys = array_append(dagOp->outkeys, RedisModule_CreateStringFromString(NULL, input->outkeys[i]));
    // RedisModule_RetainString(NULL, input->outkeys[i]);
    // dagOp->outkeys = array_append(dagOp->outkeys, input->outkeys[i]);
  }

  dagOp->outTensors = (RAI_Tensor **)array_new(RAI_Tensor *, array_len(input->outTensors));
  if (!(dagOp->outTensors)) {
    return REDISMODULE_ERR;
  }
  for (int i=0; i<array_len(input->outTensors); i++) {
    dagOp->outTensors = array_append(dagOp->outTensors, RAI_TensorGetShallowCopy(input->outTensors[i]));
  }

  dagOp->mctx = input->mctx;
  dagOp->sctx = input->sctx;
  dagOp->devicestr = RedisModule_Strdup(input->devicestr);
  dagOp->duration_us = input->duration_us;
  dagOp->result = input->result;

  RAI_InitError(&dagOp->err);
  if (!(dagOp->err)) {
    return REDISMODULE_ERR;
  }
  if (input->err->code != RAI_OK) {
    RAI_SetError(dagOp->err, input->err->code, input->err->detail);
  }

  dagOp->argv = (RedisModuleString **)array_new(RedisModuleString *, input->argc);
  if (!(dagOp->argv)) {
    return REDISMODULE_ERR;
  }
  for (int i=0; i<input->argc; i++) {
    dagOp->argv = array_append(dagOp->argv, RedisModule_CreateStringFromString(NULL, input->argv[i]));
    // RedisModule_RetainString(NULL, input->argv[i]);
    // dagOp->argv = array_append(dagOp->argv, input->argv[i]);
  }
  dagOp->argc = input->argc;
...
int RAI_ShallowCopyDagOpResult(RAI_DagOp *input, RAI_DagOp *output) {
  ...
  if (input->err->code != RAI_OK) {
    RAI_SetError(output->err, input->err->code, input->err->detail);
  }

  for (int i=0; i<array_len(output->outTensors); i++) {
    RAI_TensorFree(output->outTensors[i]);
  }
  array_free(output->outTensors);

  output->outTensors = (RAI_Tensor **)array_new(RAI_Tensor *, array_len(input->outTensors));
  if (!(output->outTensors)) {
    return REDISMODULE_ERR;
  }
  for (int i=0; i<array_len(input->outTensors); i++) {
    output->outTensors = array_append(output->outTensors, RAI_TensorGetShallowCopy(input->outTensors[i]));
  }
